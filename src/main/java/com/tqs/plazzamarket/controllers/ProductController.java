...
public class ProductController {
    ...
    public ResponseEntity<? extends Object> listProducerProducts(@PathVariable("producer_id") String producer_id) {
        Optional<Producer> producer = producerRepository.findById(producer_id);
        System.out.println(producer_id);
        System.out.println("lista1 : " + producerRepository.findById(producer_id).get().getProducts());
        
        if(!producer.isPresent()){
            System.out.println("entrei");
            return new ResponseEntity<>(null, HttpStatus.NOT_FOUND);
        }
        System.out.println("LISTA: " + producer.get().getProducts());
